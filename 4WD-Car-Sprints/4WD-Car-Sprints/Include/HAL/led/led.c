...
en_ledError LED_init(st_leds *st_a_led)
{
	...
	st_pinConfig st_a_ledPin = {st_a_led->port, st_a_led->pin, GPIO_DIRECTION_OUTPUT, GPIO_LOW};
...
{
	en_ledError en_a_retFunction = LED_OK;
	st_pinConfig st_a_ledPin = {st_a_led->port, st_a_led->pin, GPIO_DIRECTION_OUTPUT, GPIO_HIGH};
	if (DIO_setPinStatus(&st_a_ledPin, GPIO_HIGH) == DIO_OK)
		{
			st_a_led->state = 2;
			en_a_retFunction = LED_OK;
		}
...
en_ledError LED_off(st_leds *st_a_led)
{
	en_ledError en_a_retFunction = LED_OK;
	st_pinConfig st_a_ledPin = {st_a_led->port, st_a_led->pin, GPIO_DIRECTION_OUTPUT, GPIO_LOW};
	if (DIO_setPinStatus(&st_a_ledPin, GPIO_LOW) == DIO_OK)
		{
			st_a_led->state = 1;
			en_a_retFunction = LED_OK;
		}
